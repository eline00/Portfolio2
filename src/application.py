	...
	with open(file, 'wb') as f:
		...
		while True:
			try:
				...
				if flags & drtp.FIN:
					print("FIN flag received. Sending FIN-ACK")
...
if __name__ == '__main__':
	...
	# If the port is not within (1024, 65536), an error message is printed
	...
	if args.reliability_func not in ['stop-and-wait', 'gbn', 'sr']:
		print(f'ERROR: reliability function "{args.reliability_func}" is invalid. Choose from stop-and-wait, gbn or sr')
		sys.exit(1)
	...
	if args.server:
		...
		print("ERROR: must run in either client or server mode!")
		sys.exit(1)
